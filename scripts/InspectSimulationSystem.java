...
public class InspectSimulationSystem {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String url =
                "jdbc:mysql://127.0.0.1:3306/simulation_system?characterEncoding=UTF-8&useUnicode=true&useSSL=false&tinyInt1isBit=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Shanghai";
        try (Connection connection = DriverManager.getConnection(url, "root", password);
            ...
            printRows(
                    statement,
                    "select id, username, realname, org_code, status, del_flag, work_no from sys_user order by create_time desc limit 10");
            printRows(statement, "select id, role_name, role_code from sys_role order by create_time desc limit 20");
            printRows(
                    statement,
                    "select id, parent_id, name, url, component, menu_type, perms, sort_no, hidden, status from sys_permission where del_flag = 0 order by sort_no limit 30");
            printRows(
                    statement,
