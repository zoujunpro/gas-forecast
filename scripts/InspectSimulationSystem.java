...
public class InspectSimulationSystem {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection(url, "root", password);
            ...
            printRows(statement, "select u.username, r.role_code, r.role_name from sys_user u join sys_user_role ur on ur.user_id = u.id join sys_role r on r.id = ur.role_id order by u.username, r.role_code limit 50");
            printRows(statement, "select u.username, d.depart_name, d.org_code from sys_user u join sys_user_depart ud on ud.user_id = u.id join sys_depart d on d.id = ud.dep_id order by u.username limit 50");
        }
    }
