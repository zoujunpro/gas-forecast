...
public class InspectFinalTables {
    public static void main(String[] args) throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection(url, "root", "mysql2026");
            ...
            printRows(
                    statement,
                    "select region_code, region_name, region_type, created_by_name from base_region_tb where remark like 'winner-agent%' order by id limit 12");
            printRows(
                    statement,
                    "select customer_code, customer_name, industry_name, region_name, created_by_name from base_customer_tb order by id limit 5");
            printRows(
                    statement,
                    "select batch_no, region_name, best_model, mape, wmape, rmse, created_by_name from model_train_batch_tb where batch_no like 'WGTRAIN-%' order by id");
        }
    }
